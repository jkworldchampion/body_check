...
def MobileNetV3(stack_fn,
  ...
  if input_tensor is None:
    img_input = layers.Input(batch_size=batch_size, shape=input_shape)
  else:
...
def MobileNetV3Small(input_shape=None,
                     ...
                     dropout_rate=0.2,
                     classifier_activation='softmax',
                     include_preprocessing=True, c=None, attention_model=None):
  ...
  def stack_fn(x, kernel, activation, se_ratio):
    ...
    print("1 x",x)
    print("1 attention_model.outputs[0]",attention_model.outputs[0])
    x = tf.concat([x, attention_model.outputs[0]], axis=3)
    print("2 x",x)
    ...
    print("7 x",x)
    x = tf.concat([x, attention_model.outputs[1]], axis=3)
    ...
    x = _inverted_res_block(x, 6, depth(96), kernel, 1, se_ratio, activation,
                            10)
    print("10 x",x)
    x = tf.concat([x, attention_model.outputs[2]], axis=3)
    return x

  return MobileNetV3(stack_fn, 1024, batch_size, input_shape, alpha, 'small', minimalistic,
