...
def MobileNetV3(input_layer=None, type="large", input_shape=(224, 224, 3), classes_number=1000, width_multiplier=1.0,
                divisible_by=8, l2_reg=2e-5, dropout_rate=0.2, name="MobileNetV3", attention_model= None, num_seg_channels=0, num_keypoints=0):
    ...
    for i, params in enumerate(spec):
        ...
        if i == 0:
            ...
            if num_seg_channels != 0 and i == 8:
                x = tf.keras.layers.Conv2DTranspose(filters=num_seg_channels, kernel_size=3, strides=2, padding="same")(output)
                x = tf.keras.layers.PReLU()(x)
                x = tf.keras.layers.Conv2DTranspose(filters=num_seg_channels, kernel_size=3, strides=2, padding="same")(x)
                x = tf.keras.layers.PReLU()(x)
                x = tf.keras.layers.Conv2DTranspose(filters=num_seg_channels, kernel_size=3, strides=2, padding="same")(x)
                segs = tf.keras.layers.PReLU()(x)

            if attention_model is not None :
                if i == 3 :
                    output = tf.concat([output, attention_model.outputs[0]], axis=3)
                elif i == 8 :
                    output = tf.concat([output, attention_model.outputs[1]], axis=3)
                elif i == 12 :
                    output = tf.concat([output, attention_model.outputs[2]], axis=3)

        if (type == "small" and i == 14) or (type == "large" and i == 18):
            output = CusDropout(dropout_rate=dropout_rate)(output)

    joints = tf.keras.layers.Conv2D(filters=num_keypoints, kernel_size=2, activation=None)(output)
    outputs = tf.reshape(joints, (-1,num_keypoints))

    # outputs = CusReshape(out=classes_number)(output)

    if attention_model is not None :
        inputs = [inputs, attention_model.input]
    
    if num_seg_channels != 0 :
        outputs = [outputs, segs]
