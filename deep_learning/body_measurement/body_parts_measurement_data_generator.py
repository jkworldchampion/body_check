...
class BodyPartsMeasurementDataGenerator(tf.keras.utils.Sequence) :
    def __init__(self, config=None, data_type="train"):
        ...
        self.input_shape = config['input_shape']   # input shape
        self.seg_shape = config['seg_shape']       # segmentation shape
        self.path_classes = config['path_classes'] # path to classes
        self.type_backbone = config['type_backbone'] # type of backbone
        self.type_loss_fn = config['type_loss_fn'] # type of loss function
        self.epochs = config['epochs']             # number of epochs
        self.is_with_seg = config['is_with_seg']   # whether to use segmentation
        self.path_dataset = config['path_dataset'] # path to dataset
        self.data_type = data_type                 # type of data
        self.shuffle = config['shuffle']           # whether to shuffle data
        self.is_normalized = config['is_normalized']      # whether to normalize data
        self.type_attention = config['type_attention']    # type of attention
        self.num_category_bmi = config['num_category_bmi'] # number of categories for bmi
        self.num_category_height = config['num_category_height'] # number of categories for height
        self.has_filename = config['has_filename']  # whether to return filename
    ...
    def get_data(self, idx_id):
        ...
        img = cv2.resize(img, (self.input_shape[0], self.input_shape[1]))  # model input size에 맞게 resize

        if self.is_normalized :  # 이미지 정규화
            img = img / 255.0
        ...
        body_parts_measurement = body_parts_measurement[1:32]            # 키 이후의 31개만 사용
    ...
    def get_list_data(self) :  
        ...
        for user_dir in list_user_dirs :
            try :
                ...
                # df = pd.read_csv(path_user_csv, encoding = "utf-8")
